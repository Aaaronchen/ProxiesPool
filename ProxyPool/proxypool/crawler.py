...
'''    
    def crawl_xundaili(self):
        start_url = 'http://api.xdaili.cn/xdaili-api//greatRecharge/getGreatIp?spiderId=1353e7d72b2i5faeb2b040062df0b798&orderno=YZ201811282197zxYtMT&returnType=1&count=10'
        req = requests.get(url=start_url)
        content = req.content.decode("utf-8")
        content = content.replace("\r\n",',')[:-1]
        content = content.split(',')
...
'''
    def crawl_daxiangdaili(self):
        start_url = 'http://tpv.daxiangdaili.com/ip/?tid=557105430555141&num=50'
        proxies = requests.get(url=start_url).text
        address_port = proxies.replace('\r','').split('\n')
